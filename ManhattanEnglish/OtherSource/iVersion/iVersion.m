...
@property (nonatomic) BOOL checkFromEnterForeground;
...
@synthesize checkFromEnterForeground;
...
/**************************************************************************
 ...
 备注      :	N/A
 *************************************************************************/
- (void)showErrorView:(NSString*)errorInfo
{
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:errorInfo
                                                       delegate:self cancelButtonTitle:self.okButtonLabel
...
- (iVersion *)init
{
    if ((self = [super init]))
    {
        ...
        self.okButtonLabel = [self localizedStringForKey:@"确定"];
        ...
        self.inThisVersionTitle = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"];//add by cafei 12.9.21
        self.checkFromEnterForeground = NO;
        ...
        @autoreleasepool
        {
            ...
            if (data)
            {
                ...
                if (bundleID && [bundleID isEqualToString:self.applicationBundleID])
                {
                    ...
                    {
                        versions = [NSDictionary dictionaryWithObject:@"" forKey:latestVersion];
                    }
                    ...
                    if (!newerVersionAvailable && !self.checkFromEnterForeground)
                    {
...
- (void)applicationLaunched
{
    ...
    {
        self.checkFromEnterForeground = YES;
